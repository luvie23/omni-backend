...
class CertificationVerificationController extends Controller
{
    public function verify(Request $request)
    {
        ...
        /*
        |--------------------------------------------------------------------------
        | 3) Company / Contractor Search - DISABLED
        |--------------------------------------------------------------------------
        |
        | Contractor/company searching has been temporarily disabled.
        |
        */
        ...
        foreach ($candidates as $c) {
            ...
            if (
                ...
                (
                    $score === $bestScore &&
                    strlen($nameNorm) < strlen($this->normalize($best->company_name))
                )
            ) {
                $best = $c;
                $bestScore = $score;
